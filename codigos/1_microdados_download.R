...
# Available at: https://github.com/rafapereirabr/Brazilian_Census
# By, Rafael Pereira
...
dir.create(file.path("./dados/", "ibge"))
dir.create(file.path("./dados/ibge", "censo"))
dir.create(file.path("./dados/ibge/censo", "dados_txt2010"))
dir.create(file.path("./dados/ibge/censo", "dados_csv2010"))
dir.create(file.path("./dados/ibge/censo", "documentacao2010"))

destfolder <- "./dados/ibge/censo/dados_txt2010/"
UFlist <- c("SP2_RM")
...
for (i in UFlist){
  ...
  td <- paste0("./dados/ibge/censo/dados_txt2010/", i, ".zip")
  print(i)
  download.file(tf, td, mode="wb")
}

# unzip all Files
filenames <- list.files("./dados/ibge/censo/dados_txt2010", pattern=".zip", full.names=TRUE)
lapply(filenames, unzip, exdir = "./dados/ibge/censo/dados_txt2010")
...
download.file(file_url,"./dados/ibge/censo/documentacao2010/Documentacao.zip", mode="wb")
unzip("./dados/ibge/censo/documentacao2010/Documentacao.zip", exdir="./dados/ibge/censo/documentacao2010", junkpaths=T)

######## Prepare Documentation files to read .txt -----------------------------------------------------

# Open variables layout from Excel file
dic_dom <- read_excel("./dados/ibge/censo/documentacao2010/Layout_microdados_Amostra.xls", sheet =1, skip = 1)
dic_pes <- read_excel("./dados/ibge/censo/documentacao2010/Layout_microdados_Amostra.xls", sheet =2, skip = 1)
dic_mor <- read_excel("./dados/ibge/censo/documentacao2010/Layout_microdados_Amostra.xls", sheet =4, skip = 1)
...
data_files  <- list.files(path="./dados/ibge/censo/dados_txt2010",
                          recursive=T,
                          pattern="Dom",
                          full.names=T)
...
fwrite(temp, file="./dados/ibge/censo/dados_csv2010/censo2010_RMSPdom.csv")
rm(temp,readDOM); gc()
...
for (i in 1:length(data_files)){
  ...
  # Save national data set as a '.csv' file
  fwrite(temp, file="./dados/ibge/censo/dados_csv2010/censo2010_RMSPpes.csv", append = T)
  rm(temp); gc()
}
proc.time() - ptm   # Stop the clock
